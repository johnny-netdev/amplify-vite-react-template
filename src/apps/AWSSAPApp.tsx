        ...
        {showAdmin && (
          <div style={v.adminPanel}>
            <div style={{ marginBottom: '2rem', borderBottom: '1px solid #333', paddingBottom: '2rem' }}>
                <h3 style={{ color: '#ff9900', marginTop: 0, fontSize: '0.9rem', fontFamily: 'monospace' }}>INPUT_NEW_DATA_MODULE</h3>
                    ...
                    {existingVisuals.map((vis) => (
                        <div key={vis.id} style={v.purgeItem}>
                            <span style={{ fontSize: '0.8rem', fontFamily: 'monospace' }}>
                                <b style={{ color: '#ff9900', marginRight: '10px' }}>[{vis.domain}]</b> {vis.title}
                            </span>
                            {/* ⭐️ EXACT CSS FROM CISSPApp.tsx */}
                            <button 
...
const v = {
  ...
  adminPanel: { 
    background: 'rgba(0, 0, 0, 0.8)', backdropFilter: 'blur(10px)', border: '2px solid #ff9900', 
    padding: '2rem', marginBottom: '2rem', borderRadius: '12px' 
  },
  input: { padding: '10px', background: '#000', color: '#ff9900', border: '1px solid #333', fontFamily: 'monospace' },
